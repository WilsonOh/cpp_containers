...
class simple_string {
  ...
  /* make the string iterator a friend class so that it can access the private members of simple string */
...
public:
  ...
  /* copy assignment operator: takes in an lvalue reference to another simple string, makes a copy of it
   * and swaps its contents with the current simple string.
   * Hence, the "copy and swap" idiom
   */
  simple_string &operator=(const simple_string &other) noexcept;
  /* move assignment operator: takes in a rvalue reference to a simple string, basically does the same thing as
  the move constructor just that we are using the "=" assignment operator to move the rvalue instead */
  ...
  /* internal swap function which takes in a lvalue reference instead of a const reference,
   * used for the copy assignment operator 
   */
 ...
 */
class simple_string_iterator {
  /* tags for the compiler to know what kind of iterator this is. not inheriting from
  the base iterator class as it is getting deprecated */
...
public:
  ...
  bool operator!=(const simple_string_iterator &other) const;
  /* the mutable non-const iterator has both the const deference and non-const deference operators */
  char & operator*();
...
class simple_string_const_iterator {
  ...
  /* MAIN difference: const iterator has a CONST reference to a simple string instead */
  const simple_string &_s;
  mutable std::size_t _pos;
  /* IMPT: the const iterator constructor takes in a const reference to a simple string! */
...
public:
  ...
  bool operator!=(const simple_string_const_iterator &other) const;
  /* Another difference: the const iterator only has the const deference operator! */
  const char & operator*() const;
};
#endif //
